import AnnualPlanner from "../models/AnnualPlannerModel.js";
import { supabase } from "../utils/supabase.js";

export const AnnualPlannerUpload = async (req, res) => {
  try {
    if (!req.file) {
    ...
    const newImage = new AnnualPlanner({
      image: {
...
export const getAnnualPlanner = async (req, res) => {
  try {
    const images = await AnnualPlanner.find().sort({ createdAt: -1 }); // Latest first
...
export const updateAnnualPlanner = async (req, res) => {
  try {
    const { fileName } = req.body; // Assuming only fileName needs to be updated

    const updatedImage = await AnnualPlanner.findByIdAndUpdate(
      req.params.id,
...
// Delete image
export const deleteAnnualPlanner = async (req, res) => {
  try {
    const image = await AnnualPlanner.findById(req.params.id);
    if (!image) {
      return res.status(404).json({ error: "Image not found" });
    }
    ...
    // Delete from MongoDB
    await AnnualPlanner.findByIdAndDelete(req.params.id);
